...
public class InputManager : MonoBehaviour {
	...
	private Vector3 newMarkPos;
	private Vector3 markFwd;
	...
	public void StartPath() {
		if (!AugmentedImageVisualizer.Instance.CanGetPos()) return;
		markFwd = AugmentedImageVisualizer.Instance.GetCenterFoward();
		markPos = AugmentedImageVisualizer.Instance.GetCenterPos();
	...
	public void path1Save() {
		...
		ARPointSavedData savedData = new ARPointSavedData();
		savedData.PrepareDate(path1Array,markPos);
		var str = JsonUtility.ToJson(savedData);
		SaveLocal(str);
		SaveCloud(str);
		reset();
		initialize();
	...
	private void SaveLocal(string strJSON) {
		PlayerPrefs.SetString("SaveARDate", strJSON);
	}
	private void SaveCloud(string strJSON) { }
	private string LoadLocal()
	{
		return PlayerPrefs.GetString("SaveARDate",string.Empty);
	}
	private string LoadCloud() { return string.Empty; }
	public void SetNewMark() {
		if (!AugmentedImageVisualizer.Instance.CanGetPos()) return;
		newMarkPos = AugmentedImageVisualizer.Instance.GetCenterPos();
	}
	...
	public void followPath1() {
		...
		rabitVisible = true;
		var str = LoadLocal();
		ARPointSavedData data = JsonUtility.FromJson<ARPointSavedData>(str);
		if (data == null) return;
		path1Array = data.GetDate(newMarkPos);
		path1Len = path1Array.Length;
		for (int x = 0; x < path1Len; x++) {
